...
export async function loadGrowApp(page: Page) {
...
export async function clickOnLoginRegisterLink(page: Page, newpage) {
    await newpage.locator('//*[text()="Login/Register"]').isVisible()
    await newpage.locator('//*[text()="Login/Register"]').click()
}

export async function enterEmailandClickOnContinue(page: Page, newpage) {
...
export async function navigateToInboxWebsite(page: Page) {
    const browser = await chromium.launch()
    const context = await browser.newContext()
...
export async function verifymailandclickOnmail(page: Page, page1) {
...
export async function enterOtp(newpage, otp) {
